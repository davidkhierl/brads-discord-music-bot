 ...
 * Spotify.
 */
export default class FrennyDJBotCopy {
	/**
	 * Frenny DJ Bot instance
	 */
	private static instance: FrennyDJBotCopy;
	 ...
	 */
	readonly commands: Collection<string, BotCommandBuilder>;
	 ...
	 */
	private static readonly rest = new REST({ version: '10' }).setToken(
	...
	private setCommandsCollection() {
		const commands = FrennyDJBotCopy.getAllCommandsInstance();
	...
	public static getAllCommandsInstance() {
		const commandFiles = fs
			.readdirSync(FrennyDJBotCopy.commandsDir)
			.filter((file) => file.match(/(\.[tj]s$)/g));

		return commandFiles.map(
			(file) =>
				new (require(path.join(
					FrennyDJBotCopy.commandsDir,
					file
				)).default)() as BotCommandBuilder
		);
	}
	...
	public static async deployCommandsToGuild(
		...
		const commands = FrennyDJBotCopy.getAllCommandsToJSON();

		FrennyDJBotCopy.rest
			.put(
	...
	public static getAllCommandsToJSON(): RESTPostAPIApplicationCommandsJSONBody[] {
		return FrennyDJBotCopy.getAllCommandsInstance().map((command) =>
	 ...
	 */
	public static getInstance(): FrennyDJBotCopy {
		if (!this.instance) {
			this.instance = new FrennyDJBotCopy();
		}
	...
	public static getClient(): Client<boolean> {
		return FrennyDJBotCopy.getInstance().client;
	}
	...
	public static getPlayer(): Player {
		return FrennyDJBotCopy.getInstance().player;
	}
}
